...
quota | awk 'NR>2 {
    ...
    percent=$(echo "$line" | awk -F'[(),%]' '{print $3}' | awk '{printf "%d",$1}')
...
# --- PREAGUSE KAUSTA JAOTUS (top 10) ---
echo -e "\n${BOLD}Praeguse kausta jaotus (top 10):${RESET}"
du -s .[!.]* * 2>/dev/null | sort -rn | head -n 10 | awk -v RED="$RED" -v YELLOW="$YELLOW" -v GREEN="$GREEN" -v RESET="$RESET" '
{
    size=$1
    name="./"$2
    hum_size=""
